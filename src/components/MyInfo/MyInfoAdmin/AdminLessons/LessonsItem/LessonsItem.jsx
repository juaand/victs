...
export default function LessonsItem({lesson, onClick}) {
    ...
    return (
        <div className={`lesson-item col-sm-2 col-6 LessonsItem ${lesson?.capacity === 0 && "waiting-list"} ${lesson?.gym === null && "instructor-lesson"}`} onClick={() => onClick(lesson)}>
            ...
            <span>
                ...
                {lesson?.gym === null ? <div className="lesson __gym">{lesson?.address}</div> : <div className="lesson __gym">{lesson?.gym?.user?.name}</div>}
